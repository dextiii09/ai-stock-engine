...
const SYMBOLS = ['NIFTYBEES.NS', 'WIPRO.NS', 'RELIANCE.NS', 'ONGC.NS'];

export const IndianMarket = () => {
  const navigate = useNavigate();
  const [regime, setRegime] = useState<any>(null);
  const [portfolioData, setPortfolioData] = useState<any>(null);
  const [opportunities, setOpportunities] = useState<any[]>([]);
  const [selectedSymbol, setSelectedSymbol] = useState('NIFTYBEES.NS');
  ...
  useEffect(() => {
    const fetch_ = async () => {
      try {
        const res = await fetch(`${API_BASE}/indian/data/live/${selectedSymbol}`);
  ...
  // Market data poll (regime, portfolio, opportunities, positions)
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [regimeRes, portfolioRes, holdingsRes, oppRes] = await Promise.all([
          fetch(`${API_BASE}/indian/data/regime`),
          fetch(`${API_BASE}/indian/portfolio/money-tracker`),
          fetch(`${API_BASE}/indian/portfolio/holdings`),
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card border border-border p-6 rounded-3xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-500 via-white/20 to-emerald-500" />
        ...
        <div>
          <h1 className="font-display text-4xl font-bold tracking-tight mb-2 flex items-center gap-3">
            <Globe className="w-10 h-10 text-emerald-500" /> Indian Market
          </h1>
          <p className="text-muted-foreground text-sm max-w-xl">
            Live NSE/BSE market data — prices, signals &amp; AI regime. Auto-trade runs on the AutoTrader page.
          </p>
        </div>
        <button
          onClick={() => navigate('/autotrader')}
          className="flex items-center gap-2 px-6 py-3 bg-emerald-500 text-white rounded-2xl font-bold text-sm hover:bg-emerald-600 hover:scale-105 transition-all shadow-lg shadow-emerald-500/20 whitespace-nowrap"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-3xl p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold">₹{balance.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
            <p className={`text-xs font-bold ${profitLoss >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              {profitLoss >= 0 ? '+' : ''}₹{profitLoss.toLocaleString('en-IN', { maximumFractionDigits: 2 })} PnL
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-border p-6 rounded-3xl space-y-4">
            <div className="flex items-center justify-between border-b border-border/50 pb-4">
              <h3 className="font-display font-bold text-lg flex items-center gap-2">
                <Activity className="w-5 h-5 text-emerald-500 animate-pulse" /> Live Price Monitor
              </h3>
                ...
                {SYMBOLS.map(sym => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                      selectedSymbol === sym ? 'bg-emerald-500/10 text-emerald-500' : 'text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    {sym.split('.')[0]}
                  </button>
            ...
            {livePrice ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-background/50 border border-border/50 rounded-2xl">
                <div>
                  ...
                  <p className="font-display text-lg font-bold">₹{livePrice.price?.toLocaleString('en-IN')}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className={`font-display text-lg font-bold ${
                    livePrice.institutional_flow === 'BULLISH' ? 'text-emerald-500' :
          ...
          <div className="bg-card border border-border rounded-3xl p-6">
            <h3 className="font-display font-bold text-xl mb-4 flex items-center gap-2">
              <Layers className="w-5 h-5 text-emerald-500" /> Active Holdings
            </h3>
                  ...
                  <tbody>
                    {activeHoldings.map((h: any) => (
                      <tr key={h.symbol} className="border-b border-border/30 last:border-none text-sm">
                        ...
                        <td className="py-4">₹{h.entry_price?.toLocaleString('en-IN')}</td>
                        <td className="py-4">₹{h.current_price?.toLocaleString('en-IN')}</td>
                        <td className="py-4 text-right font-bold text-emerald-500">₹{h.value?.toLocaleString('en-IN')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* Right — Signals + AI Coach */}
        <div className="xl:col-span-1 space-y-6">

          {/* Opportunities */}
          <div className="bg-card border border-border rounded-3xl p-6">
            <h3 className="font-display font-bold text-lg mb-5 flex items-center gap-2">
              <Crosshair className="w-5 h-5 text-emerald-500" /> NSE Signals
            </h3>
            <div className="space-y-3">
              {opportunities.length === 0 ? (
                <p className="text-xs text-muted-foreground italic">Scanning NSE/BSE universe...</p>
              ) : (
                opportunities.slice(0, 6).map((opp, idx) => (
                  <div key={idx} className="p-3 bg-background border border-border rounded-xl flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-lg text-xs font-bold ${
                      opp.signal === 'BUY'  ? 'bg-emerald-500/20 text-emerald-500' :
          ...
          <div className="bg-[#0c0c0e] border border-border rounded-3xl p-6 relative overflow-hidden">
            ...
            <h3 className="font-display font-bold text-lg mb-3 flex items-center gap-2 text-emerald-400">
            ...
            {opportunities.length > 0 ? (
              <div className="space-y-3 relative z-10">
                ...
                <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
                  <p className="text-xs text-emerald-400 italic font-bold">Recommendation:</p>
                  <p className="text-sm text-emerald-400">{opportunities[0].recommendation}</p>
                </div>
              </div>
            ) : (
              <p className="text-xs text-muted-foreground">Waiting for first NSE scan to complete...</p>
            )}
          </div>
          ...
          {regime?.active_strategy && (
            <div className="bg-card border border-border rounded-3xl p-6">
              <h3 className="font-display font-bold text-lg mb-3 flex items-center gap-2">
                <Zap className="w-5 h-5 text-emerald-500" /> Active Strategy
              </h3>
              <p className="text-sm font-bold text-emerald-500 mb-1">{regime.active_strategy.name}</p>
